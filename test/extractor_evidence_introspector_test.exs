defmodule Ex4pmCore.ExtractorEvidenceIntrospectorTest do
  ...
  test "missing capability is unsupported rather than empty success" do
    assert {:error, {:unsupported, :capability_unavailable, {Probe, :missing, 0}}} =
             Introspector.call(Probe, :missing, [])
  end

  test "raised reflection is a typed refusal" do
    assert {:error, {:refused, :introspection_raised, RuntimeError}} =
             Introspector.call(Probe, :explode, [])
