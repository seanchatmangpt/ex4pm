defmodule Ex4pmCore.CapsuleIndependenceSourceTest do
  ...
  test "source and witness require exact bounded identities" do
    ...
    assert {:error, {:refused, :inexact_subject}} =
             Source.new("owner/repo@main", "p", "r", "a", "f")

    assert {:ok, source} =
             Source.new("owner/repo@" <> sha, "producer", "run", "artifact", "family")
    ...
    assert {:error, {:refused, :invalid_evidence_scope}} =
             Witness.new(source.id, attempt, :pass, :planet, now, "bad")
  end
end
