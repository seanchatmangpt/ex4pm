defmodule Explore.AStar do
  ...
  defp loop([{_,g,node,path}|rest],seen,goal,edges,h) do
    if MapSet.member?(seen,node), do: loop(rest,seen,goal,edges,h), else: begin = for {^node,to,w}<-edges, do: {g+w+h.(to),g+w,to,path++[to]}; loop(Enum.sort(begin++rest),MapSet.put(seen,node),goal,edges,h) end
  end
end
