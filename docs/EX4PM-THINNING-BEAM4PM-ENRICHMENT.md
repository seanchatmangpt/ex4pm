...
- **The false ggen-pack citation (§1.2) needs its own resolution independent of this migration.** 17 of 33 leaf adapters cite a nonexistent `ex4pm-wasm4pm-bindings-pack`. Whether that citation is corrected, removed, or the pack is actually built is a separate decision from whether the execution surface moves to beam4pm — noted here so it isn't silently carried forward or silently dropped during the Phase 3 deletion.
...
- No decision is made here about the false `ex4pm-wasm4pm-bindings-pack` citation's resolution (§7) beyond flagging it.
