...
the architecture; it is grounded in a freshly gathered per-app capability inventory of
...
- **parse** — the raw shape is normalized into the canonical IR. `Ex4pm.OCEL.normalize/1`
  and `Ex4pm.XES.parse/2` (both in `ex4pm_core`) both converge on one
  `%Ex4pm.EventLog{}` struct regardless of source format.
- **route** — a request is matched against a closed capability table
  (`Ex4pm.Information.Registry.public_capabilities/0` in `ex4pm_information`) or an
  engine candidate list (`Ex4pm.Engine.Registry.engines/0` in `ex4pm_engine`). Routing
...
- **admit | refuse** — the routed request is checked against a schema
  ...
  (`Ex4pm.Refusal.new/3`, `ex4pm_core`), never an exception or a silent drop.
- **construct** — analytical (non-mutating) work happens here: discovery, conformance,
  ...
  `Ex4pm.plan/2`, `Ex4pm.cmca/2` (all in the top-level `ex4pm` app). Construct produces
...
- **BRCE** — the sole boundary allowed to invoke a state-changing callback.
  `Ex4pm.Evidence.BRCE.execute/5` (`ex4pm_evidence`) admits authority, writes a
...
- **DO** — the actual state-changing effect, invoked only from inside a BRCE-wrapped
  closure. `Ex4pm.operate/3` (top-level `ex4pm`) is the only public entrypoint that
  reaches DO, via `Ex4pm.Runtime.compile/1` + `Ex4pm.Runtime.execute/3`
  (`ex4pm_runtime`), which itself routes every individual POWL task through
  `BRCE.execute/5`.
...
- **replay** — receipts are not trusted at face value; `Ex4pm.Evidence.Replay.verify/1`
  ...
  `Ex4pm.replay/2` (top-level `ex4pm`) is the public entrypoint.
...
`Ex4pm.Information.execute/2` (`ex4pm_information`) is the single Reactor graph that
...
`Ex4pm.Evidence.BRCE.execute/5` is the only function in the entire umbrella (per the
...
`Ex4pm.Qualification.Scanners.BrceEnforcer` meaningful: it AST-scans the umbrella for
...
`Ex4pm.Standing.rank/1`, `.min/2`, `.to_string/1` (`ex4pm_core`) give this vocabulary
...
executed run with a specific standing — the whole `ex4pm_qualification` app exists to
...
## The umbrella app dependency architecture

Apps are ordered bottom-to-top by what each layer is allowed to assume exists below
it. Each boundary exists to keep one concern from silently leaking into another:

- **`ex4pm_contracts`** — the canonical semantic contract surface (ontology TTL, SHACL
  shapes, WIT world, receipt JSON Schema) as four fixed, hashed artifact identities.
  `Ex4pm.Contracts.verify/0` recomputes a combined `contract_hash` and checks each
  artifact's bytes for required terms, so drift between the declared contract and the
  actual files on disk is a caught event, not a silent divergence. Depends on nothing
  but the standard library.
- **`ex4pm_core`** — the canonical semantic IR and shared evidence primitives:
  `EventLog`, `ProcessIR`, `POWL`, OCEL2/OLAP views, `Ex4pm.Core.Hash`, `Ex4pm.Refusal`,
  `Ex4pm.Subject`, `Ex4pm.Standing`. This is the bottom of the umbrella and the one app
  `~/xaas` is meant to depend on directly as a library — it exists as a separate app
  specifically so a sibling project can pull in canonical types without pulling in
  Phoenix, Ash, Broadway, or Reactor.
- **`ex4pm_evidence`** — receipts, the BRCE boundary, replay verification, and
  standards-format evidence generation (EARL, SOSA, PROV-O, DCAT, SPDX). Separate from
  `ex4pm_core` because evidence/receipt machinery is a distinct concern from the IR
  itself — a receipt references a subject hash, it does not need to know how to parse
  XES.
- **`ex4pm_engine`** — the DfCM engine registry and all 25 candidate engine
  implementations. Kept separate from `ex4pm_core`/`ex4pm_evidence` because engines are
  swappable execution strategies over the same IR — this app can grow new engine
  wrappers without ever touching the canonical types they operate on.
- **`ex4pm_runtime`** — Reactor-based POWL compilation and BRCE-gated OTP execution,
  plus distributed multi-node execution. Separate from `ex4pm_engine` because runtime
  execution (DO) is a fundamentally different authority domain from engine selection
  (CONSTRUCT) — see the SELECT/CONSTRUCT/DO section above.
- **`ex4pm_domain`** — the Ash/ETS semantic control-plane projection (Agent, Event,
  Object, ConformanceResult, Receipt, ProcessModel, Intervention, EngineCapability).
  Kept as its own app so that persistence is an explicit, one-directional projection
  (`Ex4pm.Domain.Projector`) from core/evidence structs — nothing upstream depends on
  Ash, and persistence can never silently become the source of semantic truth.
- **`ex4pm_information`** — the Reactor-based information plane and closed capability
  protocol. Separate from `ex4pm_domain` and `ex4pm` because it owns the *wire
  protocol* (request/response envelopes, JSON-safety, a static non-dynamic capability
  registry) rather than the orchestration logic itself.
- **`ex4pm_stream`** — Broadway-based backpressured, acknowledged ingestion. Kept
  separate because ingestion has its own operational concerns (backpressure,
  idempotency by sequence number, Prometheus telemetry) that are orthogonal to
  analytical operations.
- **`ex4pm_qualification`** — the anti-cheat/qualification suite. Deliberately outside
  the observation -> ... -> standing runtime path — it audits and independently
  re-verifies the claims that path produces, and must not be a dependency of anything
  it audits, or it could not be trusted as an independent check.
- **`ex4pm`** — the public orchestration API (`Ex4pm.ingest/2`, `.discover/2`,
  `.conform/3`, `.simulate/2`, `.optimize/3`, `.plan/2`, `.cmca/2`, `.operate/3`,
  `.stream/2`, `.capabilities/2`, `.differential/5`, `.replay/2`). This is the one app
  that is allowed to know about every layer below it and compose them into a single
  facade; nothing else in the umbrella is allowed to depend on `ex4pm` itself, which
  is what keeps the dependency graph acyclic.
- **`ex4pm_web`** — the Phoenix/LiveView HTTP surface: `POST /api/v1/ocel/events`,
  health/readiness endpoints, live dashboards, AshAdmin. It is a thin projection over
  `ex4pm_stream`/`ex4pm_domain` — it defines no orchestration logic of its own.
- **`ex4pm_cli`** — the escript/CLI projection of `ex4pm`'s public API
  (`Ex4pm.CLI.main/1`: `doctor`, `contracts`, `discover`, `discover-xes`), plus a
  separate Ash-resource-blueprint code-generation Mix task
  (`mix ex4pm.gen.blueprint`).

The general rule: an app boundary exists wherever one authority domain, one
...
another if left in the same app.

## The DfCM engine candidate model

`Ex4pm.Engine.Registry.engines/0` (`ex4pm_engine`) holds a fixed ordered list of 25
...
- `/Users/sac/ex4pm/CLAUDE.md` — project overview, commands, umbrella architecture
...
- `/Users/sac/ex4pm/docs/REACTOR_INFORMATION_PLANE.md` — the `ex4pm_information`
  Reactor flow in detail
- `/Users/sac/ex4pm/docs/ROADMAP-xaas-integration.md` — the `ex4pm_core`/
  `ex4pm_contracts` library-dependency boundary for `~/xaas`
