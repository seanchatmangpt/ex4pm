defmodule Ex4pmCore.CapsuleIndependenceEngineTest do
  ...
  alias Ex4pmCore.CapsuleGraph.Currentness.{Attempt, Context, Lease, Subject, Witness}
  alias Ex4pmCore.CapsuleGraph.Currentness.Engine, as: CurrentEngine
  alias Ex4pmCore.CapsuleGraph.Independence.{Provenance, Relation, Source}
  alias Ex4pmCore.CapsuleGraph.Independence.Engine, as: IndependentEngine
  ...
  test "current capsule needs independent evidence and moved context invalidates the predecessor attempt" do
    ...
    assert {:ok, %{replay: true}} = CurrentEngine.qualify(attempt, [context], currentness_witness, 15)
    ...
    assert {:ok, %{standing: :unknown}} =
             IndependentEngine.qualify([a, b], [wa, wb], provenance, MapSet.new(), attempt.id, now, 2)
    ...
    assert {:ok, %{standing: :partial_alive, replay: true, actuation_performed: false}} =
             IndependentEngine.qualify([a, b], [wa, wb], provenance, independent, attempt.id, now, 2)
    ...
    assert {:error, {:refused, :stale_target}} = CurrentEngine.qualify(attempt, [moved], moved_witness, 15)
  end
end
