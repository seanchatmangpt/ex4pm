defmodule Ex4pmCore.CapsuleGraph.PipelineTest do
  ...
  alias Ex4pmCore.CapsuleGraph.{Capability, Candidate, Evidence, Pipeline, Replay, Runtime, Subject, Transport}
  ...
  test "selects exact planner capsule, preserves alternatives, receipts construct, never actuates" do
    ...
    {:ok, subject} = Subject.new("seanchatmangpt/ex4pm-plan", "e5da34c8b42089f1ebb1fd2306d95f0c4986f8c3")
    ...
    {:ok, candidate} = Candidate.new("ex4pm-plan", subject, runtime, transport, [capability], [e2e, replay])

    assert {:ok, result} = Pipeline.construct([candidate], [capability], %{graph: :admitted}, %{cost: 2})
