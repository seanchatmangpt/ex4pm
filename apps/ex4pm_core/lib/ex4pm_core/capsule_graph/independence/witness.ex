  ...
  def new(source_id, attempt_id, outcome, scope, observed_at, evidence_id)
    ...
    cond do
      byte_size(source_id) != 64 ->
        {:error, {:refused, :invalid_source_id}}

      byte_size(attempt_id) != 64 ->
        {:error, {:refused, :invalid_attempt_id}}

      outcome not in @outcomes ->
        {:error, {:refused, :invalid_evidence_outcome}}

      scope not in @scopes ->
        {:error, {:refused, :invalid_evidence_scope}}

      not match?(%DateTime{}, observed_at) ->
        {:error, {:refused, :naive_evidence_time}}

      String.trim(evidence_id) == "" ->
        {:error, {:refused, :missing_evidence_id}}
