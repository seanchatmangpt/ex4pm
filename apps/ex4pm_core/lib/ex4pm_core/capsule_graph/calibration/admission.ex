defmodule Ex4pmCore.CapsuleGraph.Calibration.Admission do
  ...
  @spec admit(Model.t() | struct(), Witness.t() | struct(), DateTime.t(), pos_integer()) ::
          :ok | {:error, term()}
  def admit(%Model{} = model, %Witness{} = witness, %DateTime{} = now, min_trials)
      when is_integer(min_trials) and min_trials > 0 do
    cond do
      model.source_id != witness.source_fingerprint ->
        {:error, {:refused, :source_calibration_mismatch}}

      DateTime.compare(witness.observed_at, now) == :gt ->
        {:error, {:refused, :future_evidence}}

      model.support < min_trials ->
        {:error, {:refused, :insufficient_calibration_support}}
