defmodule Ex4pmCore.CapsuleGraph.Calibration.Witness do
  ...
  @type outcome :: :pass | :fail | :pending | :unknown | :unsupported
  @type t :: %__MODULE__{
          attempt_id: String.t(),
  ...
  @spec new(String.t(), String.t(), atom(), DateTime.t(), String.t()) ::
          {:ok, t()} | {:error, term()}
  def new(attempt_id, source_fingerprint, outcome, %DateTime{} = observed_at, scope)
      when is_binary(attempt_id) and is_binary(source_fingerprint) and outcome in @outcomes and
             is_binary(scope) do
    if String.trim(attempt_id) != "" and String.trim(source_fingerprint) != "" and
         String.trim(scope) != "" do
      {:ok,
       %__MODULE__{
