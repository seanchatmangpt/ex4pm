defmodule Ex4pmCore.CapsuleGraph.Currentness.Attempt do
  ...
  def new(subject, base_digest, target_digest, ordinal, nonce, lease)
      when is_binary(base_digest) and is_binary(target_digest) and is_integer(ordinal) and ordinal >= 0 and is_binary(nonce) and nonce != "" do
    body = %{subject: subject, base_digest: base_digest, target_digest: target_digest, ordinal: ordinal, nonce: nonce, lease: lease}
