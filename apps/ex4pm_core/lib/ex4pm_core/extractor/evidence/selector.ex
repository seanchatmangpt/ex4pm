defmodule Ex4pmCore.ProcessIR.Extractor.Evidence.Selector do
  ...
  def select(candidates) when is_list(candidates) do
    viable = Enum.filter(candidates, fn candidate -> Map.get(@rank, candidate.standing, -1) > 0 end)

    case Enum.sort_by(viable, fn candidate -> {-Map.fetch!(@rank, candidate.standing), candidate.name} end) do
