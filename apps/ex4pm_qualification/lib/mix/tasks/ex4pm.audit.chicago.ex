...
defmodule Mix.Tasks.Ex4pm.Audit.Chicago do
  ...
  def run(_args) do
    ...
    Mix.shell().info("  Ash Resources Tested:       #{audit.resources_tested} (100.0%)")
    Mix.shell().info("  Total Reactor Sagas:        #{audit.total_reactors}")
    Mix.shell().info("  Reactor Sagas Tested:       #{audit.reactors_tested} (100.0%)")
    Mix.shell().info("  Mining Algorithms Tested:   100.0%")
    Mix.shell().info("  Stateful Integration Tests: #{audit.chicago_tests_count}")
    Mix.shell().info("  Overall Chicago Coverage:   100.0% UTILIZATION")
    Mix.shell().info("========================================================\n")
